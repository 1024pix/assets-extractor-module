  ...
  await Promise.all(
    remoteUrls.map(async (raw) => {
      try {
        ...
        console.warn(`Unable to extract asset "${raw}"`)
        console.error(e)
      }
    })
  )
...
module.exports = async function (moduleOptions) {
  const options = {
    ...DEFAULT_OPTIONS,
    ...this.options['assets-extractor-module'],
    ...moduleOptions
  }
  ...
  this.nuxt.hook('generate:page', async (page) => {
    try {
      ...
      console.warn(`Failed to extract asset links for route "${page.route}"`)
      console.error(e)
    }
    return page
  })
  ...
  this.nuxt.hook('generate:routeCreated', async ({ route }) => {
    ...
    try {
      ...
      console.warn(`Failed to extract asset links for route "${route}"`)
      console.error(e)
    }
  })
}
